...
fn main() {
    ...
    let tele_bot = thread::spawn(move || { teleprint::bot::run_bot(Arc::clone(&config), Arc::clone(&users_table)); });
    let imap_bot = thread::spawn(move || { teleprint::mailbot::run_bot(Arc::clone(&a_config), Arc::clone(&a_users)); });
